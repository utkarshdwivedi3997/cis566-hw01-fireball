...
void main()
{
    ...
    float rot = smoothstep(-2.0, 2.0, vs_Pos.y);
    fs_Pos = vec4(rotateY(vec3(vs_Pos),  rot * 17.0), 1.0);    // distance based rotation for "twisting"
    fs_Pos = vec4(rotateY(vec3(fs_Pos), u_Speed * u_Time * 0.08), 1.0);    // constant rotation for the whole sphere

    vec4 modelposition = u_Model * fs_Pos;   // Temporarily store the transformed vertex positions for use below

    // Calculate the UVs
    // vec3 dir = normalize(vec3(vs_Pos));  // - vec3(0.0) but we don't need that
    // float latitude = asin(dir.y);
    // float longitude = atan2(dir.z, dir.x) * 0.5 * PI;

    fs_UV = vec2(0.5 + 0.5 * atan(vs_Pos.x, -vs_Pos.z) / PI, 1.0 - acos(vs_Pos.y) * PI);
